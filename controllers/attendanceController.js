...
exports.checkIn = async (req, res, next) => {
  ...
  try {
    ...
    const existingRecord = await Attendance.findOne({ userId, checkInTime: { $gte: todayStart.toDate(), $lte: todayEnd.toDate() } });
    if (existingRecord) {
    ...
    const onTimeDeadline = todayStart.clone().hour(9).minute(0).second(0); // 9:00 AM
    ...
    next(error); // Pass to a centralized error handler if you have one
  } finally {
    cleanupFile(req.file);
  }
};
